...
function collectAllUrls() {
  ...
  urls.push({ path: '/halong-bay-cruise/', priority: '0.9', changefreq: 'monthly' });
  urls.push({ path: '/sapa-trekking-tour/', priority: '0.9', changefreq: 'monthly' });
  urls.push({ path: '/mekong-delta-tour/', priority: '0.9', changefreq: 'monthly' });
...
function generateSitemapXML(urls, locale) {
  ...
  const urlElements = urls.map(url => {
    ...
    const alternates = LOCALES.map(alt => {
      const altPath = alt === DEFAULT_LOCALE
        ? url.path
        : (url.path === '/' ? `/${alt}/` : `/${alt}${url.path}`);
      return `    <xhtml:link rel="alternate" hreflang="${alt}" href="${SITE_URL}${altPath}" />`;
    }).join('\n');

    // x-default points to EN (the non-default locale path)
    const enPath = url.path === '/' ? '/en/' : `/en${url.path}`;
    const xDefault = `    <xhtml:link rel="alternate" hreflang="x-default" href="${SITE_URL}${enPath}" />`;
...
/**
 * Generate a combined sitemap XML containing all locales' URLs in one file.
 */
function generateCombinedSitemapXML(urls) {
  const currentDate = new Date().toISOString();
...
function generateSitemap() {
  ...
  // Generate combined sitemap.xml with ALL locales
  const combinedXml = generateCombinedSitemapXML(urls);
  fs.writeFileSync(path.join(publicDir, 'sitemap.xml'), combinedXml);
  const combinedCount = urls.length * LOCALES.length;
  console.log(`  sitemap.xml: ${combinedCount} URLs (all locales combined)`);
  totalUrls += combinedCount;

  // Also generate per-locale sitemaps for the sitemap index
  const sitemapIndexes = [{ filename: 'sitemap.xml', lastmod: currentDate }];

  LOCALES.forEach(locale => {
    if (locale !== DEFAULT_LOCALE) {
      const sitemapFilename = `sitemap-${locale}.xml`;
      const xml = generateSitemapXML(urls, locale);
      fs.writeFileSync(path.join(publicDir, sitemapFilename), xml);
      sitemapIndexes.push({ filename: sitemapFilename, lastmod: currentDate });
      console.log(`  ${sitemapFilename}: ${urls.length} URLs (${locale.toUpperCase()})`);
    }
  });

  // Generate sitemap index pointing to combined + per-locale sitemaps
...
`;
  ...
  console.log(`\nTotal URLs in sitemap.xml: ${combinedCount} (${urls.length} NL + ${urls.length} EN)`);
